...
public class PFishBeginnerShop extends PAbstractInventory {

    @Override
    public void initializeLayout() {
        PType pT = PType.getPType(PLayout.class);
        if (pT == null) return;
        this.layout = (PLayout) pT.getPFileSafely("fish_beginner_shop");
    }
    ...
    public void command(Player p, String s, ItemStack iS) {
        ...
        if (s.contains("buy")) {
            ...
            if (pFP == null) return;
            if (pB.sell == null && pB.sellIS == null) return;
            if (pFP.balance >= pB.price) {
                if (pB.sellIS != null) {
                    ItemStack sellIS = pB.sellIS.clone();
                    sellIS.setAmount(pB.amount);
                    pFP.giveItemStack(sellIS);
                } else {
