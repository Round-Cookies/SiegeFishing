...
public class PFishBook extends PAbstractInventory {
    ...
    public void initializeLayout() {
        ...
        if (pT == null) return;
        this.layout = (PLayout) pT.getPFileSafely("fish_book");
    ...
    public List<ItemStack> finalizeItemStackList(Player p, List<ItemStack> list) {
        ...
        PType pT2 = PType.getPType(PFishCollectable.class);
        ...
        List<PFishCollectable> fishCollectableList = new ArrayList<>();
        for (PFile pF : pT2.getPFileList()) {
            fishCollectableList.add((PFishCollectable) pF);
        }
        fishCollectableList.sort((o1, o2) -> {
            if (o1.level.level > o2.level.level) return 1;
            if (o1.level.level < o2.level.level) return -1;
            return o1.name.compareTo(o2.name);
        });
        List<ItemStack> itemStackList = new ArrayList<>();
        for (PFishCollectable fishCollectable : fishCollectableList) {
            if (pFP.fishCountCounter.containsKey(fishCollectable) && pFP.fishWeightCounter.containsKey(fishCollectable) && pFP.fishRegionCounter.containsKey(fishCollectable)) {
                int count = pFP.fishCountCounter.get(fishCollectable);
                float weight = pFP.fishWeightCounter.get(fishCollectable);
                PFishRegion pFC = pFP.fishRegionCounter.get(fishCollectable);
                itemStackList.add(fishCollectable.getItemStackForBook(count, weight, pFC));
            } else {
