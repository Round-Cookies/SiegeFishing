...
public class PInsectBook extends PAbstractInventory {
    ...
    public void initializeLayout() {
        ...
        if (pT == null) return;
        this.layout = (PLayout) pT.getPFileSafely("insect_book");
    ...
    public List<ItemStack> finalizeItemStackList(Player p, List<ItemStack> list) {
        ...
        PType pT2 = PType.getPType(PInsectCollectable.class);
        ...
        List<PInsectCollectable> insectCollectableList = new ArrayList<>();
        for (PFile pF : pT2.getPFileList()) {
            insectCollectableList.add((PInsectCollectable) pF);
        }
        insectCollectableList.sort((o1, o2) -> {
            if (o1.level.level > o2.level.level) return 1;
            if (o1.level.level < o2.level.level) return -1;
            return o1.name.compareTo(o2.name);
        });
        List<ItemStack> itemStackList = new ArrayList<>();
        for (PInsectCollectable insectCollectable : insectCollectableList) {
            if (pFP.insectCountCounter.containsKey(insectCollectable)) {
                int count = pFP.insectCountCounter.get(insectCollectable);
                int starCount = pFP.insectStarCountCounter.get(insectCollectable);
                itemStackList.add(insectCollectable.getItemStackForBook(count, starCount));
            } else {
